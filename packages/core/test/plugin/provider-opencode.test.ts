...
describe("OpencodePlugin", () => {
  ...
  // aiand fork: `console login` exports the org key as OPENCODE_CONSOLE_TOKEN, which
  // isn't in the provider's known env list, so the plugin sets it as request.body.apiKey
  // directly (rather than the public key) and keeps paid models enabled.
  it.effect("uses OPENCODE_CONSOLE_TOKEN (console login) as credentials", () =>
    withEnv({ OPENCODE_API_KEY: undefined, OPENCODE_CONSOLE_TOKEN: "sk-test" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.opencode)).request.body.apiKey).toBe("sk-test")
        expect(required(yield* catalog.model.get(ProviderV2.ID.opencode, ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )
