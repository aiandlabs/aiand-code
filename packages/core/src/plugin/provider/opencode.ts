...
// aiand fork: default the managed console/device-auth server to api.aiand.com
// (overridable via OPENCODE_CONSOLE_URL — matches account.ts's defaultConsoleUrl).
// Upstream value is https://console.opencode.ai. The credential's metadata.server
// still overrides this at request time once an account is connected.
const defaultServer = process.env["OPENCODE_CONSOLE_URL"] ?? "https://api.aiand.com"
...
export const OpencodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    yield* ctx.catalog.transform((catalog) => {
      ...
      if (!item) return
      // aiand fork: `console login` stores the account in the SQLite store and
      // server.ts exports its org key as OPENCODE_CONSOLE_TOKEN — a path separate
      // from the integration-connection store that `connected` reflects, so honor
      // that env token here too. The runner resolves env-enabled keys at request time.
      const consoleToken = process.env["OPENCODE_CONSOLE_TOKEN"]
      const hasKey = Boolean(
        process.env.OPENCODE_API_KEY || connected || consoleToken || item.provider.request.body.apiKey,
      )
      catalog.provider.update(item.provider.id, (provider) => {
        // aiand fork: route managed model calls through our gateway, not OpenCode Zen.
        provider.request.body.baseURL = aiandGatewayUrl
        // aiand fork: console login's org key isn't in the provider's known env
        // list, so the runner can't resolve it by name — set it as the request
        // apiKey directly so model calls authenticate against our gateway.
        if (consoleToken && !provider.request.body.apiKey) provider.request.body.apiKey = consoleToken
