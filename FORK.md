...
| `packages/core/src/plugin/provider/opencode.ts` | Sets the `opencode` provider gateway baseURL → `https://api.aiand.com/v1` (env-overridable via `OPENCODE_GATEWAY_URL`); upstream resolves this from models.dev (`https://opencode.ai/zen/v1`). Also honors `OPENCODE_CONSOLE_TOKEN` in the v2 catalog transform (counts as a key + marks the provider `enabled: {via:"env"}`) — upstream's 1.17 v2 catalog only enables providers via credential/env, so without this the `/api/config` bridge never reaches the model picker and login yields no models | Keep the `baseURL` line + const and the `consoleToken` block; take upstream changes to the rest of the plugin |
| `packages/core/test/plugin/provider-opencode.test.ts` | Adds the `OPENCODE_CONSOLE_TOKEN` test; pins `OPENCODE_CONSOLE_TOKEN: undefined` in the no-credential cases | Keep our test + the env pins; take upstream's new cases |
